...
void DMA2_C1_Handler(void) __attribute__ ((weak, alias("Default_Handler")));
void DMA2_C2_Handler(void) __attribute__ ((weak, alias("Default_Handler")));
void DMA2_C3_Handler(void) __attribute__ ((weak, alias("Default_Handler")));
void DMA2_C4_5_Handler(void) __attribute__ ((weak, alias("Default_Handler")));
...
uint32_t vector_tbl[] __attribute__((section(".isr_vector_tbl"))) = {
	...
	(uint32_t)&DMA2_C1_Handler,
	(uint32_t)&DMA2_C2_Handler,
	(uint32_t)&DMA2_C3_Handler,
	(uint32_t)&DMA2_C4_5_Handler
};
void Default_Handler(void)
{
	while (1)
...
void Reset_Handler(void)
{
	...
	for (int i = 0; i < ((uint32_t) &_edata) - ((uint32_t) &_sdata); i++)
	{
		*p_dest_mem++ = *p_src_mem++;
	}

	p_dest_mem = (uint32_t *)_sbss;
	for (int i = 0; i < ((uint32_t) &_ebss) - ((uint32_t) &_sbss); i++)
	{
		*p_dest_mem++ = 0;
	}
